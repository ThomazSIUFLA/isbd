...
$liv = $control->detalharLivro($codLivro);
$reg = $liv->fetch_assoc();
$editora = $control->buscaNomeEditora($codLivro);
$qtd = $control->retornaQtd($codLivro);
$aut = $control->listarAutor($codLivro);
$autor = $aut->fetch_assoc();
$detExemp = $control->detalExemplares($codLivro);
$detalExemplares = $detExemp->fetch_assoc();
?>
<!doctype html>
<html lang="pt-br">
...
<body style="background-image: url('../img/fundo.jpg');">
    <span class="d-block p-2 bg-gradient-dark text-info font-weight-bold h2 text-center">Alterar livro<h3 class="text-success"> <?= $reg['tituloLivro'] ?></h3></span>
        ...
        <form method="POST" action="" class="container" style="width:100%; height: 400px; overflow: auto;">
            ...
            <div class="form-group row">
                ...
                <select id="editora" class="form-control col-md-4 mb-3 ml-2" name="edit" required>
                    <option value="<?= $reg['cnpjEditora'] ?>"><?= $reg['cnpjEditora'] . " - " . $editora ?></option>
            ...
            <div class="form-group formulario row">
                ...
                <input type="text" class="form-control col-6" name="titulo" required value="<?= $reg['tituloLivro'] ?>">
                <label for="exampleInputEmail1">Categoria do Livro</label>
                <select name="cat" id="cat" class="col-2 form-control">
                    <?php
                    if ($reg['tipoLivro'] == 'L') { ?>
                        <option value="L">Literatura</option><?php
                                                            } else { ?>
                        <option value="D">Didático</option><?php
                                                            }
                                                            ?>
            ...
            <div class="row">
                ...
                <input type="number" max-lenght="4" name="anoPublic" min="1900" max="2030" class="col-1 mb-3 form-control" required value="<?= $reg['anoPublicLivro'] ?>">
                <label for="exampleInputEmail1">Edição</label>
                <input type="number" name="edicao" class="col-1 mb-4 form-control" value="<?= $reg['edicaoLivro'] ?>" required>
                <label for="exampleInputEmail1" id="disc">Estilo</label>
                <input type="text" name="disc" class="col-1 mb-4 form-control" value="<?= $reg['estiloLivroLiteratura'] . $reg['disciplinaLivroDidatico'] ?>" required>
            </div>
            <div class="row">
                <label for="exampleInputEmail1">isbn</label>
                <input type="text" max-lenght="13" name="isbn" class="col-3 mb-3 form-control" placeholder="Digite o código ISBN" value="<?= $reg['codIsbnLivro'] ?>" required>
                <label for="exampleInputEmail1">Quantidade de exemplares</label>
                <input type="number" max-lenght="13" name="quantidade" class="col-md-2 mb-3 form-control" placeholder="Digite a quantidade" required value="<?= $qtd['qtdtotal'] ?>">
                <label for="autor">Autor</label>
                <input type="text" class="form-control col-3" name="autor" value="<?= $autor['nomeAutor'] ?>">
            </div>
            <div class="row col-12">
                <label for="exampleInputEmail1">Localização</label>
                <div class="col-2">
                    <label for="exampleInputEmail1">Seção</label>
                    <input type="text" max-lenght="13" name="secao" class="mb-3 form-control" required value="<?= $detalExemplares['secaoLocalExemplar'] ?>">
                </div>
                <div class="col-1">
                    <label for="exampleInputEmail1">Corredor</label>
                    <input type="number" max-lenght="13" name="corr" class="mb-3 form-control" value="<?= $detalExemplares['corredorLocalExemplar'] ?>" required>
                </div>
                <div class="col-1">
                    <label for="exampleInputEmail1">Prateleira</label>
                    <input type="number" max-lenght="13" name="prat" class="mb-3 form-control" value="<?= $detalExemplares['prateleiraLocalExemplar'] ?>" required>
                </div>
                <div class="col-4 ml-2">
                    ...
                    <div>
                        ...
                        <input type="radio" id="ms" name="tipoEmp" value="M" checked>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary btn-lg ml-1" id="btn" style="width: 300px; height: 40px;">Salvar alterações</button>
            </div>
...
if ($_POST) {
    ...
    $control->alterarLivro($codLivro, $titulo, $cat, $ano, $edicao, $atr, $qtd, $cnpj, $secao, $corr, $prat, $tipoEmp, $autor);
?><script>
